...
class DocProjectAssignment(BaseModel):
    """Request to assign doc to a project."""
    project_id: str
    chapter_id: Optional[str] = None


class DocContentUpdate(BaseModel):
    """Request to update doc content."""
    content: str
    language: str = "en"


class DocTitleUpdate(BaseModel):
    """Request to update doc title."""
    title: str = Field(..., min_length=1, max_length=200)


class DocEvaluationRequest(BaseModel):
    """Request to evaluate a doc."""
    language: str = "en"
...
def _extract_images_from_content(content: str) -> list[str]:
    ...
    import re
    # Match markdown image syntax: ![alt](path) and extract the path
...
async def list_docs(
    ...
    """List all docs for current user."""
    doc_ids = storage.list_docs()

    docs = []
    for doc_id in doc_ids:
        languages = storage.list_doc_languages(doc_id)

        # Count images from the first available language content
        screenshot_count = 0
        if languages:
            content = storage.get_doc_content(doc_id, languages[0])
            if content:
                screenshot_count = _count_images_in_content(content)

        # Get creation time from folder
        doc_dir = storage.docs_dir / doc_id
        created_at = None
        if doc_dir.exists():
        ...
        if metadata:
            ...
            title = _get_doc_title(metadata, doc_id)

        # Get evaluation status for each language
        evaluations: dict[str, LanguageEvaluation] = {}
        version_storage = VersionStorage(user_id, doc_id)
...
async def get_doc(
    ...
    # Extract image paths from the document content (not file listing)
    screenshots = _extract_images_from_content(content)
    ...
    return DocDetail(
        ...
        content=content,
        language=language,
        screenshots=screenshots,  # Image paths extracted from content
        source_video=source_video,
        document_format=document_format,
...
@router.get("/{doc_id}/languages")
async def get_doc_languages(
    doc_id: str,
    user_id: CurrentUser,
    storage: UserStorageDep,
) -> dict:
    """Get available languages for a doc."""
    languages = storage.list_doc_languages(doc_id)
    if not languages:
        raise HTTPException(status_code=404, detail="Doc not found")

    return {"doc_id": doc_id, "languages": languages}


@router.put("/{doc_id}/content")
async def update_doc_content(
    doc_id: str,
    update: DocContentUpdate,
    user_id: CurrentUser,
    storage: UserStorageDep,
) -> dict:
    """Update doc content.
    ...
    try:
        ...
        saved_path = storage.save_doc_content(
            doc_id=doc_id,
...
async def update_doc_title(
    ...
    try:
        ...
        storage.update_doc_metadata(doc_id, {"title": new_title})
...
async def clone_doc(
    doc_id: str,
    clone_request: CloneDocRequest,
    user_id: CurrentUser,
    storage: UserStorageDep,
) -> DocSummary:
    """Clone a doc to a different document format.
    ...
    source_metadata = storage.get_doc_metadata(doc_id)
    ...
    if clone_request.document_format == source_format:
        raise HTTPException(
            status_code=400,
            detail=f"Doc is already in '{source_format}' format. Choose a different format."
        )
    ...
    try:
        # Clone the manual
        new_doc_id, new_doc_dir = storage.clone_doc(
            source_doc_id=doc_id,
            target_format=clone_request.document_format,
        ...
        # Build DocSummary response
        languages = storage.list_doc_languages(new_doc_id)
        screenshots = storage.list_screenshots(new_doc_id)
        new_metadata = storage.get_doc_metadata(new_doc_id)
        ...
        return DocSummary(
            id=new_doc_id,
            title=new_metadata.get("title", new_doc_id) if new_metadata else new_doc_id,
...
@router.post("/{doc_id}/screenshots/extract")
async def extract_new_screenshot(
    ...
    NOTE: This route MUST be defined before /{doc_id}/screenshots/{filename}
    to avoid "extract" being matched as a filename.
    """
    from ...agents.video_doc_agent.tools.video_tools import extract_screenshot_at_timestamp
    from ...agents.video_doc_agent.utils.metadata import get_video_path_by_id
    ...
    video_path = get_video_path_by_id(doc_dir, video_id)
    ...
    screenshots_dir = storage.docs_dir / doc_id / "screenshots"
    ...
    try:
        from ...agents.video_doc_agent.tools.video_tools import get_video_metadata
    ...
    try:
        ...
        return {
            ...
            "timestamp": timestamp,
            "video_id": video_id,
            "url": f"/api/docs/{doc_id}/screenshots/{new_filename}",
...
@router.get("/{doc_id}/screenshots/{filename}")
async def get_screenshot(
    ...
    screenshot_path = storage.docs_dir / doc_id / "screenshots" / filename
    screenshot_path = screenshot_path.resolve()
    screenshots_dir = (storage.docs_dir / doc_id / "screenshots").resolve()
...
@router.post("/{doc_id}/screenshots/{filename}/replace")
async def replace_screenshot(
    ...
    screenshot_dir = storage.docs_dir / doc_id / "screenshots"
    ...
    try:
        ...
        return {
            "success": True,
            "filename": filename,
            "url": f"/api/docs/{doc_id}/screenshots/{filename}",
            "version": new_version or version_storage.get_current_version(),
        }
    except Exception as e:
...
@router.delete("/{doc_id}/screenshots/{filename}")
...
async def extract_frames(
    ...
    frames_dir = Path(tempfile.gettempdir()) / "doc_frames" / doc_id / video_id
    ...
    for i in range(count):
        ...
        try:
            ...
            frames.append({
                ...
                "url": f"/api/docs/{doc_id}/frames/{video_id}/{frame_filename}",
            })
        except Exception as e:
    ...
    from ...agents.video_doc_agent.utils.metadata import load_metadata
...
@router.get("/{doc_id}/frames/{video_id}/{filename}")
async def get_frame(
    doc_id: str,
    video_id: str,
    filename: str,
...
@router.post("/{doc_id}/screenshots/{filename}/from-frame")
...
@router.get("/{doc_id}/videos")
async def list_doc_videos(
    doc_id: str,
    user_id: CurrentUser,
    storage: UserStorageDep,
) -> DocVideosResponse:
    """List all videos for a doc."""
    from ...agents.video_doc_agent.utils.metadata import (
        load_metadata,
    ...
    metadata = load_metadata(doc_dir)
    if not metadata:
        raise HTTPException(status_code=404, detail="Doc metadata not found")
    ...
    optimized_video = doc_dir / "video_optimized.mp4"
    ...
    additional_videos_raw = get_additional_videos(doc_dir)
    additional_videos = []
    for video in additional_videos_raw:
        video_path = doc_dir / "videos" / video["filename"]
    ...
    return DocVideosResponse(primary=primary_info, additional=additional_videos)


@router.post("/{doc_id}/videos")
async def upload_additional_video(
    ...
    import uuid
    from ...agents.video_doc_agent.tools.video_tools import get_video_metadata
    from ...agents.video_doc_agent.tools.video_preprocessor import (
        needs_optimization,
        preprocess_video_for_analysis,
    )
    from ...agents.video_doc_agent.utils.metadata import add_additional_video
    ...
    videos_dir = storage.get_doc_videos_dir(doc_id)
...
@router.delete("/{doc_id}/videos/{video_id}")
async def delete_additional_video(
    ...
    video_info = get_additional_video_by_id(doc_dir, video_id)
    if not video_info:
        raise HTTPException(status_code=404, detail="Video not found")

    # Delete file from disk
    video_path = doc_dir / "videos" / video_info["filename"]
    if video_path.exists():
        video_path.unlink()

    # Remove from metadata
    removed = remove_additional_video(doc_dir, video_id)
    if not removed:
...
@router.get("/{doc_id}/videos/{video_id}/stream")
...
async def delete_doc(
    ...
    return {"status": "moved_to_trash", "doc_id": doc_id}


# ==================== Project Assignment ====================


@router.put("/{doc_id}/project")
async def assign_doc_to_project(
    doc_id: str,
    assignment: DocProjectAssignment,
    user_id: CurrentUser,
    storage: UserStorageDep,
    project_storage: ProjectStorageDep,
) -> dict:
    """Assign a doc to a project and optionally a chapter.
    ...
    if current_project_id and current_project_id != assignment.project_id:
        try:
            project_storage.remove_doc_from_project(current_project_id, doc_id)
        except ValueError:
            pass

    # Add to new project
    project_storage.add_doc_to_project(
...
async def remove_doc_from_project(
    ...
    """Remove a doc from its current project."""
    doc_dir = storage.docs_dir / doc_id
    if not doc_dir.exists():
        raise HTTPException(status_code=404, detail="Doc not found")
    ...
    if not project_id:
        raise HTTPException(status_code=400, detail="Doc is not assigned to a project")

    project_storage.remove_doc_from_project(project_id, doc_id)

    return {"doc_id": doc_id, "removed_from_project": project_id}
...
async def get_doc_tags(
    ...
    """Get tags for a doc."""
    metadata = project_storage._get_doc_metadata(doc_id)
    if metadata is None:
        raise HTTPException(status_code=404, detail="Doc not found")

    return {"doc_id": doc_id, "tags": metadata.get("tags", [])}


@router.post("/{doc_id}/tags")
async def add_doc_tags(
    doc_id: str,
    tags: list[str],
    user_id: CurrentUser,
    project_storage: ProjectStorageDep,
    ...
    """Add tags to a doc."""
    for tag in tags:
        project_storage.add_tag_to_doc(doc_id, tag)

    return {"doc_id": doc_id, "added_tags": tags}
...
async def remove_doc_tag(
    ...
    project_storage.remove_tag_from_doc(doc_id, tag)
    return {"doc_id": doc_id, "removed_tag": tag}
...
async def list_doc_versions(
    ...
    """List all versions for a doc, optionally filtered by language."""
    doc_dir = storage.docs_dir / doc_id
    if not doc_dir.exists():
        raise HTTPException(status_code=404, detail="Doc not found")
    ...
    if language:
        ...
        for v in versions:
            ...
            if version_num == current_version:
                ...
                manual_path = doc_dir / language / "manual.md"
...
async def get_version_content(
    ...
    """Get doc content for a specific version."""
    doc_dir = storage.docs_dir / doc_id
    if not doc_dir.exists():
        raise HTTPException(status_code=404, detail="Doc not found")
...
@router.get("/{doc_id}/versions/{version}/screenshots/{filename}")
async def get_version_screenshot(
    ...
    """
    version_dir = storage.docs_dir / doc_id / "versions" / f"v{version}"
    ...
    if mapping_path.exists():
        ...
        store = ScreenshotStore(storage.docs_dir / doc_id)
...
async def restore_doc_version(
    ...
    """Restore a doc to a previous version.
...
@router.post("/{doc_id}/versions/snapshot")
...
async def export_doc(
    doc_id: str,
    export_request: DocExportRequest,
    user_id: CurrentUser,
    storage: UserStorageDep,
) -> dict:
    """Export doc to specified format (PDF, Word, or HTML).
    ...
    """
    from ...export.doc_exporter import create_doc_exporter
    ...
    # Validate format
    if export_request.format.lower() not in ("pdf", "word", "docx", "html", "chunks", "markdown", "md"):
        raise HTTPException(
            status_code=400,
            detail=f"Unsupported format: {export_request.format}. Supported: pdf, word, html, markdown, chunks"
        )

    # Verify manual has content for the requested language
    content = storage.get_doc_content(doc_id, export_request.language)
    if not content:
        raise HTTPException(
            status_code=404,
            detail=f"Doc content not found for language: {export_request.language}"
        )
    ...
    try:
        ...
        if export_request.format.lower() == "chunks":
            ...
            exporter = create_chunks_exporter(user_id=user_id, doc_id=doc_id)
            ...
            return {
                ...
                "download_url": f"/api/docs/{doc_id}/exports/{output_file.name}",
        ...
        if (
            ...
            exporter = ManualTemplateExporter(user_id=user_id, doc_id=doc_id)
...
async def list_doc_exports(
    doc_id: str,
    user_id: CurrentUser,
    storage: UserStorageDep,
) -> dict:
    """List all previous exports for a doc."""
    doc_dir = storage.docs_dir / doc_id
    if not doc_dir.exists():
        raise HTTPException(status_code=404, detail="Doc not found")

    export_dir = doc_dir / "exports"
    if not export_dir.exists():
        return {"doc_id": doc_id, "exports": []}
    ...
    for export_file in export_dir.glob("*.*"):
        ...
        exports.append({
            ...
            "format": format_type,
            "language": language,
            "download_url": f"/api/docs/{doc_id}/exports/{export_file.name}",
    ...
    return {"doc_id": doc_id, "exports": exports}


@router.get("/{doc_id}/exports/{filename}")
async def download_doc_export(
    doc_id: str,
    filename: str,
    user_id: CurrentUser,
    storage: UserStorageDep,
):
    """Download a doc export file."""
    export_path = storage.docs_dir / doc_id / "exports" / filename
...
- Actionable recommendations
    ...
    responses={
        ...
        404: {"description": "Doc or language not found"},
...
async def evaluate_doc(
    doc_id: str,
    evaluation_request: DocEvaluationRequest,
    user_id: CurrentUser,
    storage: UserStorageDep,
) -> dict:
    """Evaluate a doc's quality using AI analysis.
    ...
    Args:
        doc_id: The unique identifier of the manual to evaluate
    ...
    content = await asyncio.to_thread(
        storage.get_doc_content, doc_id, evaluation_request.language
    )
    if not content:
        raise HTTPException(
            status_code=404,
            detail=f"Doc content not found for language: {evaluation_request.language}"
        )

    metadata = await asyncio.to_thread(storage.get_doc_metadata, doc_id)
    if not metadata:
        raise HTTPException(status_code=404, detail="Doc metadata not found")
...
''',
        # Report formats
        "incident-report": f'''  "evidence_quality": {{
    "score": <number {score_range}>,
    "explanation": "<does the report include clear visual evidence (screenshots) supporting the findings? Are the issues well-documented with photos? Is the evidence objective and factual?>"
  }},
  "severity_assessment": {{
    "score": <number {score_range}>,
    "explanation": "<is the severity level clearly stated and justified? Are immediate risks and potential consequences explained? Is the urgency appropriately communicated?>"
  }},
  "actionable_recommendations": {{
    "score": <number {score_range}>,
    "explanation": "<are recommendations specific, actionable, and prioritized? Is it clear who should do what and when? Could this report be used for work orders or insurance claims?>"
  }},
''',
        "inspection-report": f'''  "systematic_coverage": {{
    "score": <number {score_range}>,
    "explanation": "<does the report systematically cover all inspected areas/items? Is each inspection point clearly documented with visual evidence? Is coverage comprehensive?>"
  }},
  "status_clarity": {{
    "score": <number {score_range}>,
    "explanation": "<are pass/fail or condition statuses clearly stated for each item? Is the overall inspection result clear? Is the terminology consistent throughout?>"
  }},
  "findings_documentation": {{
    "score": <number {score_range}>,
    "explanation": "<are issues and observations clearly distinguished? Is visual evidence provided for each finding? Are recommendations properly prioritized?>"
  }},
''',
        "progress-report": f'''  "milestone_tracking": {{
    "score": <number {score_range}>,
    "explanation": "<are accomplishments and milestones clearly documented? Is progress visually demonstrated with before/after or current state screenshots? Is the timeline clear?>"
  }},
  "status_communication": {{
    "score": <number {score_range}>,
    "explanation": "<is the overall project status (on track/delayed/ahead) clearly stated and justified? Are issues and delays honestly reported with context?>"
  }},
  "forward_planning": {{
    "score": <number {score_range}>,
    "explanation": "<are next steps clearly defined? Is it clear what will be done next and by when? Could stakeholders understand the path forward from this report?>"
  }},
''',
    }
    ...
    format_name = format_names.get(document_format, "Document")
...
async def list_evaluations(
    ...
    """List all stored evaluations for a doc."""
    doc_dir = storage.docs_dir / doc_id
    if not doc_dir.exists():
        raise HTTPException(status_code=404, detail="Doc not found")
    ...
    return {"doc_id": doc_id, "evaluations": evaluations}


@router.get("/{doc_id}/evaluations/{version}")
