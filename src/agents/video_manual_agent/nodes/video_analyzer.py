...
def analyze_video_node(state: VideoManualState) -> Dict[str, Any]:
    ...
    1. For small videos (<15MB, <2min): Send directly as base64
    2. For larger videos: Optimize first, then decide upload method
    3. For optimized videos >20MB: Use Gemini Files API
    ...
    if needs_optimization(metadata):
        ...
        if manual_id:
            # Use the manual's output directory
            from ....storage.user_storage import UserStorage

            storage = UserStorage(user_id)
            output_dir = str(storage.get_manual_path(manual_id))
