...
def load_metadata(doc_dir: Path) -> Optional[Dict[str, Any]]:
...
def has_analysis(doc_dir: Path) -> bool:
...
def has_keyframes(doc_dir: Path) -> bool:
...
def get_cached_analysis(doc_dir: Path) -> Optional[str]:
...
def get_cached_keyframes(doc_dir: Path) -> Optional[List[Dict[str, Any]]]:
...
def get_cached_video_metadata(doc_dir: Path) -> Optional[Dict[str, Any]]:
...
def update_analysis(
    ...
    """
    metadata = load_metadata(doc_dir) or create_metadata("")
...
def update_keyframes(doc_dir: Path, keyframes: List[Dict[str, Any]]) -> None:
...
def add_language_generated(doc_dir: Path, language_code: str) -> None:
...
def has_optimized_video(doc_dir: Path) -> bool:
    ...
    """
    return (doc_dir / "video_optimized.mp4").exists()


def has_screenshots(doc_dir: Path) -> bool:
...
def get_project_id(doc_dir: Path) -> Optional[str]:
...
def get_chapter_id(doc_dir: Path) -> Optional[str]:
...
def get_tags(doc_dir: Path) -> List[str]:
...
def add_tag(doc_dir: Path, tag: str) -> None:
...
def remove_tag(doc_dir: Path, tag: str) -> None:
...
def set_tags(doc_dir: Path, tags: List[str]) -> None:
...
def get_version(doc_dir: Path) -> str:
...
def get_version_history(doc_dir: Path) -> List[Dict[str, Any]]:
    """Get version history.
...
def update_version(
    ...
    Args:
        ...
        notes: Version notes
        snapshot_dir: Path to version snapshot (relative to doc_dir)
    """
    metadata = load_metadata(doc_dir) or create_metadata("")
...
def bump_version(doc_dir: Path, bump_type: str = "patch") -> str:
    """Bump version number.
...
def get_target_audience(doc_dir: Path) -> Optional[str]:
...
def get_target_objective(doc_dir: Path) -> Optional[str]:
...
def get_source_languages(doc_dir: Path) -> Optional[Dict[str, Any]]:
...
def get_additional_videos(doc_dir: Path) -> List[Dict[str, Any]]:
...
def remove_additional_video(doc_dir: Path, video_id: str) -> bool:
...
def get_additional_video_by_id(
    ...
    """
    additional_videos = get_additional_videos(doc_dir)
    for video in additional_videos:
        if video["id"] == video_id:
            return video
    return None
...
def get_video_path_by_id(doc_dir: Path, video_id: str) -> Optional[Path]:
    ...
    if video_id == "primary":
        ...
        optimized = doc_dir / "video_optimized.mp4"
        if optimized.exists():
            return optimized
