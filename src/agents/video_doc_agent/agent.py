...
class VideoDocAgent:
    ...
    - generate_doc: Extracts screenshots and generates markdown manual

    Outputs are stored in user-specific folders:
    data/users/{user_id}/manuals/{doc_id}/
    """
    ...
    def create_manual(
        ...
        Returns:
            ...
            - doc_path: Path to generated manual
        ...
        existing_doc_id = storage.find_existing_doc(video_name)
        if existing_doc_id:
            doc_dir, doc_id = storage.get_doc_dir(doc_id=existing_doc_id)
        else:
            doc_dir, doc_id = storage.get_doc_dir(video_name=video_name)

        # Prepare initial state
        initial_state: VideoDocState = {
            "user_id": user_id,
            "doc_id": doc_id,  # Pre-generated for consistent output paths
    ...
    def get_manual_content(self, user_id: str, doc_id: str) -> Optional[str]:
        ...
        """
        storage = UserStorage(user_id)
        return storage.get_doc_content(doc_id)


def create_video_doc_agent(
    ...
    Returns:
        VideoDocAgent instance
    """
    return VideoDocAgent(model_name=model_name, use_checkpointer=use_checkpointer)
...
Manual saved to: {result.get('doc_path')}
