...
export default function ManualEditorPage() {
  ...
  const handleSaveContent = useCallback(
    async (content: string) => {
      await docs.updateContent(manualId, content, language);
  ...
  async function loadManual() {
    ...
    try {
      const [manualData, languagesData] = await Promise.all([
        docs.get(manualId, language),
        docs.getLanguages(manualId),
      ]);
      ...
      if (languagesData.languages.length > 0 && !languagesData.languages.includes(language)) {
        ...
        router.replace(`/dashboard/docs/${manualId}/edit?lang=${firstLang}`);
        return;
      }
    } catch (e) {
  ...
  function handleLanguageChange(newLang: string) {
    if (hasUnsavedChanges) {
      setPendingNavigation(`/dashboard/docs/${manualId}/edit?lang=${newLang}`);
      setShowUnsavedDialog(true);
    } else {
      router.push(`/dashboard/docs/${manualId}/edit?lang=${newLang}`);
    }
  }
  ...
  const manualsListUrl = useMemo(() => {
    ...
    return queryString ? `/dashboard/docs?${queryString}` : "/dashboard/docs";
  }, [searchParams]);
  ...
  const handleExport = useCallback(async (format: "pdf" | "word" | "html" | "chunks") => {
    try {
      const result = await docs.export(manualId, format, language);
  ...
  const saveTitle = useCallback(async () => {
    ...
    setSavingTitle(true);
    try {
      await docs.updateTitle(manualId, trimmedTitle);
      // Update local state
  ...
  const handleDirectFileSelect = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        const response = await fetch(`/api/docs/${manualId}/screenshots/${activeImage.name}/replace`, {
          method: "POST",
          body: formData,
        });
      ...
      try {
        if (source === "upload" && data.file) {
          ...
          const response = await fetch(
            `/api/docs/${manualId}/screenshots/${activeImage.name}/from-frame?timestamp=${data.timestamp}&video_id=${videoId}`,
            { method: "POST" }
          );
  ...
  const handleImageDelete = useCallback(() => {
    ...
    // fetch(`/api/docs/${manualId}/screenshots/${activeImage.name}`, { method: "DELETE" });
  }, [activeImage, recordChange, t]);
  ...
  const handlePlaceholderFrameConfirm = useCallback(
    async (timestamp: number, videoId?: string) => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/docs/${manualId}/screenshots/extract?timestamp=${timestamp}&video_id=${vid}`,
          { method: "POST" }
        );
      ...
      {loading ? (
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-muted-foreground">{t("docNotFound")}</p>
            <Link href="/dashboard/docs">
              <Button className="mt-4">{t("backToDocs")}</Button>
            </Link>
          </div>
        </div>
      ) : (
                  ...
                  <ReactMarkdown
                    ...
                    components={{
                      img: ({ src, alt }) => {
                        ...
                        const apiUrl = `/api/docs/${manualId}/screenshots/${filename}?t=${imageCacheBuster}`;
                        return (
